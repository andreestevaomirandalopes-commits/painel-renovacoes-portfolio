...
import { AgendaLauncher } from "@/components/agenda/agenda-launcher";
import { auth, signOut } from "@/auth";
import { prisma } from "@/lib/prisma";

export default async function DashboardLayout({children}:{children:React.ReactNode}) {
  const session=await auth();
  const [branding,user,newAlertCount,agendaUsers]=await Promise.all([
    prisma.appSettings.findUnique({where:{id:"default"}}),
    session?.user?.email?prisma.user.findUnique({where:{email:session.user.email},select:{id:true,name:true,role:true,imageUrl:true}}):null,
    prisma.cnpjAlert.count({where:{status:"NOVO",doNotContact:false}}),
